...
async function treino() {
	let arquivo = fs.readFileSync('cotacao-das-acoes-on-da-vale.csv', {encoding: 'utf8'});
	...
	for(let l=1; l<linhas.length; l++) {
		let celulas1 = [];
		if(qtdLinhas==(linhas.length-2)) celulas1 = ['28.12.2018', 51.00, 50.31, 51.20, 50.25];
	...
	let erro = await model.fit(x, y, {epochs: 500});
	console.log('Cotação do Tipo ON da Vale em R$ (Reais)');
