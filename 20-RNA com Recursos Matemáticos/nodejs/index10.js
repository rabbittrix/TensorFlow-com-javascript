...
function train(Inputs=[], 
	...
	for(let i=1; i<=epochs; i++) {
		...
		if(tfOutput.arraySync()[0]==Infinity) {
			train(Inputs, Target, Epochs, Activation, Bias, hiddenLayers+1, model, dense);
			break;
		}
...
function predict(Inputs=[], Config={}) {
	let result = [];
...
console.log(`0 XOR 0 = ${parseFloat(predict([0, 0], configPredict)).toFixed(0)}`);
console.log(`0 XOR 1 = ${parseFloat(predict([0, 1], configPredict)).toFixed(0)}`);
console.log(`1 XOR 0 = ${parseFloat(predict([1, 0], configPredict)).toFixed(0)}`);
console.log(`1 XOR 1 = ${parseFloat(predict([1, 1], configPredict)).toFixed(0)}`);
