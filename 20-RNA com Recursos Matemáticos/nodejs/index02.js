...
function Perceptron(Input1=0, Input2=0, Target=0, Epochs=1, Activation='sigmoid', Bias=0) {
	...
	let tfWeight3 = tf.randomUniform([1, 1]).flatten();
	let tfWeight4 = tf.randomUniform([1, 1]).flatten();

	const epochs = Number(Epochs);

	for(let i=1; i<=epochs; i++) {
		let sum = tfInput1.mul(tfWeight1).mul(tfWeight3).
				  add(tfInput2.mul(tfWeight2).mul(tfWeight4)).add(tf.scalar(Bias));
		...
		tfWeight3 = tfWeight3.add(tfInput1.mul(derivada(tfError)));
		tfWeight4 = tfWeight4.add(tfInput2.mul(derivada(tfError)));
...
Perceptron(Input1=0.5, Input2=0.5, Target=0.7, Epochs=100, Activation='sigmoid', Bias=0);
